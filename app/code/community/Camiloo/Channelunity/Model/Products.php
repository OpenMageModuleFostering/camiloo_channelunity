...
class Camiloo_Channelunity_Model_Products extends Camiloo_Channelunity_Model_Abstract
{
    ...
    private $syncPosition = 0;
    public $lastSyncProd = 0;
    protected $_storeId = 0;

    public function productCallback($args) {
        $this->syncPosition++;
        $qtyStock = $args['row']['qty'];
        $row = $args['row']['sku']. "," . intval($qtyStock) . "," . $args['row']['price'] . "*\n";
        echo $row;
    ...
    public function fullstockpricemessageAction() {
        $collection = Mage::getModel('catalog/product')->getCollection()
            ...
            ->addFieldToFilter('entity_id', array('gt' => $this->lastSyncProd))
    ...
    public function enumerateCustomAttributesForProduct($product)
    {
        ...
        foreach ($attributeNames as $k => $attr) {
            ...
            //Added the below check to skip the loop for _cache_instance_products which is not needed or else it will throw array to string conversion exceptions.
            if ('_cache_instance_products' == $attr) {
                continue;
            }
                ...
                */
                ...
                if (is_array($prodDataValue)) {
					...
					if (is_array($prodDataValue)) {
						...
						$prodDataValue = var_export($prodDataValue, true);
					}
    ...
    {
    	$storesLimit = $this->getStoresToLimitTo();
    	if (count($storesLimit) > 0) {
    		// NB Only currently limiting to one store view
    		$storeName = $storesLimit[0];
        ...
        $result = Mage::getSingleton('core/resource_iterator')->walk(
            $sql, array(array($this, 'generateCuXmlSku')), array('storeId' => $storeId),
    ...
    public function executeQueryScalar($sql, $column)
    {
