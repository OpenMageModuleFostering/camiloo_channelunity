...
class Camiloo_Channelunity_Model_Orders extends Camiloo_Channelunity_Model_Abstract
{
    ...
    public function doCreate($dataArray, $order)
    {
        ...
        try {
            ...
            /////////////////////////////////////////////
            $shippingMethod = 'channelunitycustomrate_channelunitycustomrate';
            
            if (isset($order->ShippingInfo->ServiceMethod) && $order->ShippingInfo->ServiceMethod != '')
            {
                $shippingMethod = (string) $order->ShippingInfo->ServiceMethod;
            }
...
//			$newOrder = $service->submitOrder();
        ...
        try {
			...
			$newOrder->setData('ext_order_id', (string) $order->OrderId);
    ...
    {
		$canDecreaseStock = Mage::getStoreConfig('cataloginventory/options/can_subtract');
    ...
    public function doUpdate($dataArray)
    {
        foreach ($dataArray->Orders->Order as $order) {
			...
			try {
				$orderCollection = Mage::getModel('sales/order')->getCollection()->addFieldToFilter('ext_order_id', $orderId);
				foreach ($orderCollection as $newOrder) break;
				if (is_object($newOrder)) {
            ...
            if (!$bOrderExisted)
			{
				...
				if(isset($existingOrderData['entity_id']))
				{
    ...
    public function shipmentCheck($request) {
        ...
        if (isset($request->OrderIdsToCheck)) {
            foreach ($request->OrderIdsToCheck->OrderId as $orderId) {
                ...
                echo "<Info>Checking order '$orderId'</Info>\n";
                ...
                try {
                    $orderCollection = Mage::getModel('sales/order')
                        ->getCollection()->addFieldToFilter('ext_order_id', $orderId);
                    foreach ($orderCollection as $newOrder) {
                        break;
                    }
                } catch (Exception $e) {
                }
                ...
                if (is_object($newOrder)) {
                    ...
                    echo "<Info>Order found</Info>\n";
                    
                    if ($newOrder->getState() != 'processing') {
                        $cuXml = $this->generateCuXmlForOrderStatus($newOrder);
                        echo "<Info>Order not processing</Info>\n";
                        
                        if ($cuXml) {
                            echo "<Info>Sending message to CU</Info>\n";
                            
                            $cuXml .= "<ShipmentDate>" . date("c") . "</ShipmentDate>\n";
                            $this->postToChannelUnity($cuXml, 'OrderStatusUpdate');
                        }
                        else {
                            echo "<Info>Not sending to CU</Info>\n";
                        }
                    }
                    else {
                        echo "<Info>Order is processing</Info>\n";
                    }
                }
                else {
                    echo "<Info>Order not found</Info>\n";
                }
            }
        }
        echo "</ShipmentCheck>\n";
    ...
    public function orderCancellationsCheck($request) {

        $table_prefix = Mage::getConfig()->getTablePrefix();

        $db = Mage::getSingleton("core/resource")->getConnection("core_write");

        $_sql = "CREATE TABLE IF NOT EXISTS {$table_prefix}cu_credited_orders 
 			(id int AUTO_INCREMENT PRIMARY KEY, order_id int NOT NULL, date_affected TIMESTAMP DEFAULT CURRENT_TIMESTAMP);";

        $db->query($_sql);

        $_sql = "select * from {$table_prefix}sales_flat_creditmemo a 
 			WHERE a.created_at >= DATE_SUB(NOW(), INTERVAL 1 DAY)
 			AND a.order_id NOT IN (SELECT b.order_id FROM {$table_prefix}cu_credited_orders b);";
        ...
        foreach ($result as $row) {
            ...
            $creditMemos->addFieldToFilter('order_id', $row['order_id']);
...
XML;
                ...
                try {
                    ...
                    $db->query("INSERT INTO {$table_prefix}cu_credited_orders(order_id) VALUES({$row['order_id']});");
                } catch (Exception $e) {
                    Mage::logException($e);
                }
            }
